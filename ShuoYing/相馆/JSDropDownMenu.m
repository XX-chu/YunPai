...
- (CAShapeLayer *)createIndicatorWithColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    [path addLineToPoint:CGPointMake(5.5, 7)];
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"fangqixiangying" object:nil];
...
//        UITableView *leftTableView = _leftTableView;
        ...
        if (tapIndex == _currentSelectedMenudIndex && _show) {
            ...
            [(CAShapeLayer *)self.indicators[tapIndex] setStrokeColor:RGB(108, 108, 108).CGColor];
        } else {
            ...
            if (_currentSelectedMenudIndex >= 0) {
                ...
                [(CAShapeLayer *)self.indicators[_currentSelectedMenudIndex] setStrokeColor:RGB(108, 108, 108).CGColor];
            }
            ...
            [(CALayer *)self.bgLayers[tapIndex] setBackgroundColor:[UIColor whiteColor].CGColor];
            [(CATextLayer *)self.titles[tapIndex] setForegroundColor:NavigationColor.CGColor];
            [(CAShapeLayer *)self.indicators[tapIndex] setStrokeColor:NavigationColor.CGColor];
        }
    }
}
...
- (void)animateLeftTableView:(UITableView *)leftTableView rightTableView:(UITableView *)rightTableView show:(BOOL)show complete:(void(^)())complete {
    ...
    NSInteger tableViewRowCount = [self.dataSourceArr[_currentSelectedMenudIndex] count];
    ...
    if (show) {
        ...
        if (leftTableView) {
            ...
            leftTableViewHeight = (tableViewRowCount > 5) ? (5 * leftTableView.rowHeight) : (tableViewRowCount * leftTableView.rowHeight);
        ...
        if (rightTableView) {
            ...
            rightTableViewHeight = (tableViewRowCount > 5) ? (5 * rightTableView.rowHeight) : (tableViewRowCount * rightTableView.rowHeight);
        }
...
//    UILabel *titleLabel = (UILabel*)[cell viewWithTag:1];
    ...
    if (leftOrRight == 1) {
        ...
        if (!_hadSelected && _leftSelectedRow == indexPath.row) {
            cell.backgroundColor = [UIColor whiteColor];
            titleLabel.textColor = NavigationColor;
