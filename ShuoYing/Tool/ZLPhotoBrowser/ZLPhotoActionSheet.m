...
#define kBaseViewHeight (self.maxPreviewCount ? 320 : 162)

double const ScalePhotoWidth = 1000;

typedef void (^handler)(NSArray<UIImage *> *selectPhotos, NSArray<ZLSelectPhotoModel *> *selectPhotoModels, NSArray<NSData *> *imageDatas);
...
- (void)setIsTouMing:(BOOL)isTouMing{
    _isTouMing = isTouMing;
    if (isTouMing) {
        self.frame = CGRectMake(0, 0, kViewWidth, kViewHeight);
...
- (void)setIsChildController:(BOOL)isChildController{
    _isChildController = isChildController;
    if (self.isTouMing) {
        if (isChildController) {
            self.frame = CGRectMake(0, 0, kViewWidth, kViewHeight - 40);
        }else{
            self.frame = CGRectMake(0, 0, kViewWidth, kViewHeight);
        }
    }else{
...
- (void)showWithSender:(UIViewController *)sender animate:(BOOL)animate lastSelectPhotoModels:(NSArray<ZLSelectPhotoModel *> *)lastSelectPhotoModels completion:(void (^)(NSArray<UIImage *> * _Nonnull, NSArray<ZLSelectPhotoModel *> * _Nonnull, NSArray<NSData *> *imageDatas))completion
{
    [self showPreviewPhotoWithSender:sender animate:animate lastSelectPhotoModels:lastSelectPhotoModels completion:completion];
}

- (void)showPreviewPhotoWithSender:(UIViewController *)sender animate:(BOOL)animate lastSelectPhotoModels:(NSArray<ZLSelectPhotoModel *> *)lastSelectPhotoModels completion:(void (^)(NSArray<UIImage *> * _Nonnull, NSArray<ZLSelectPhotoModel *> * _Nonnull, NSArray<NSData *> *imageDatas))completion
{
    [self showPreview:YES sender:sender animate:animate lastSelectPhotoModels:lastSelectPhotoModels completion:completion];
}

- (void)showPhotoLibraryWithSender:(UIViewController *)sender lastSelectPhotoModels:(NSArray<ZLSelectPhotoModel *> *)lastSelectPhotoModels completion:(void (^)(NSArray<UIImage *> * _Nonnull, NSArray<ZLSelectPhotoModel *> * _Nonnull, NSArray<NSData *> *imageDatas))completion
{
    [self showPreview:NO sender:sender animate:NO lastSelectPhotoModels:lastSelectPhotoModels completion:completion];
}

- (void)showPreview:(BOOL)preview sender:(UIViewController *)sender animate:(BOOL)animate lastSelectPhotoModels:(NSArray<ZLSelectPhotoModel *> *)lastSelectPhotoModels completion:(void (^)(NSArray<UIImage *> * _Nonnull, NSArray<ZLSelectPhotoModel *> * _Nonnull, NSArray<NSData *> *imageDatas))completion
{
...
- (void)show
{
    ...
    if (self.animate) {
        ...
        CGPoint fromPoint = CGPointMake(kViewWidth/2, viewHeight+kBaseViewHeight/2);
        CGPoint toPoint   = CGPointMake(kViewWidth/2, viewHeight-kBaseViewHeight/2);
...
- (void)requestSelPhotos:(UIViewController *)vc
{
    ...
    NSMutableArray *datas = [NSMutableArray arrayWithCapacity:self.arraySelectPhotos.count];
    ...
    for (int i = 0; i < self.arraySelectPhotos.count; i++) {
        ...
        [[ZLPhotoTool sharePhotoTool] requestImageForAsset:model.asset scale:scale resizeMode:PHImageRequestOptionsResizeModeExact completion:^(UIImage *image ,NSData *data)  {
            strongify(weakSelf);
            if (image) [photos replaceObjectAtIndex:i withObject:[self scaleImage:image]];
            if (data) [datas replaceObjectAtIndex:i withObject:data];
            ...
            [hud hide];
            [strongSelf done:photos withImageDatas:datas];
            [strongSelf hide];
...
- (void)done:(NSArray<UIImage *> *)photos withImageDatas:(NSArray<NSData *> *)datas
{
    if (self.handler) {
        self.handler(photos, self.arraySelectPhotos.copy, datas);
        self.handler = nil;
    }
}
...
- (void)presentVC:(UIViewController *)vc
{
    ...
    [nav.navigationBar setBackgroundImage:[self imageWithColor:kRGB(71, 205, 187)] forBarMetrics:UIBarMetricsDefault];
            ...
            [[ZLPhotoTool sharePhotoTool] saveImageToAblum:image completion:^(BOOL suc, PHAsset *asset) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (suc) {
                        ...
                        model.asset = asset;
                        model.localIdentifier = asset.localIdentifier;
                        [[ZLPhotoTool sharePhotoTool] requestImageForAsset:model.asset scale:1.0 resizeMode:PHImageRequestOptionsResizeModeExact completion:^(UIImage *image, NSData *data) {
                                strongSelf.handler(@[[strongSelf scaleImage:image]], @[model], @[data]);
                            }];
