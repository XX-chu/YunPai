...
#define CollectionName [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleDisplayName"]
...
- (void)saveImageToAblumWithUrl:(NSURL *)contentUrl completion:(void (^)(BOOL suc, PHAsset *asset))completion{
    PHAuthorizationStatus status = [PHPhotoLibrary authorizationStatus];
    if (status == PHAuthorizationStatusDenied) {
        if (completion) completion(NO, nil);
    } else if (status == PHAuthorizationStatusRestricted) {
        ...
        __block PHObjectPlaceholder *placeholderAsset=nil;
        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
            PHAssetChangeRequest *newAssetRequest = [PHAssetChangeRequest creationRequestForAssetFromImageAtFileURL:contentUrl];
            placeholderAsset = newAssetRequest.placeholderForCreatedAsset;
        } completionHandler:^(BOOL success, NSError * _Nullable error) {
            if (!success) {
                if (completion) completion(NO, nil);
...
- (void)requestImageForAsset:(PHAsset *)asset scale:(CGFloat)scale resizeMode:(PHImageRequestOptionsResizeMode)resizeMode completion:(void (^)(UIImage *image ,NSData *data))completion
{
    PHImageRequestOptions *option = [[PHImageRequestOptions alloc] init];
//    option.resizeMode = resizeMode;//控制照片尺寸
    option.networkAccessAllowed = YES;
...
//            NSData *data = UIImageJPEGRepresentation([UIImage imageWithData:imageData], scale==1?sca:sca/2);
            NSLog(@"imageData - %lu",[imageData length] / 1000);
