...
typedef NS_ENUM(NSUInteger, XLPhotoBrowserStyle){
    ...
    /**
     * 顶部一个索引UILabel,顶部一个下载按钮
     */
    XLPhotoBrowserStyleDownload =4
};
...
typedef void(^BrowserDownloadImageBlock)(NSInteger currentIndex);
 ...
 *  当前显示的图片位置索引 , 默认是0
 */
@property (nonatomic, copy) BrowserDownloadImageBlock downloadBlock;
