...
- (void)setUpToolBars
{
    ...
    downloadButton.frame = CGRectMake(self.bounds.size.width - 60, CGRectGetMinY(indexLabel.frame), 50, 30);
...
- (void)setUpBrowserStyle
{
    switch (self.browserStyle) {
        ...
        case XLPhotoBrowserStyleSimple:
        {
            ...
            break;
        case XLPhotoBrowserStyleDownload:
        {
            self.indexLabel.hidden = NO;
            self.saveButton.hidden = YES;
            self.pageControl.hidden = YES;
...
- (void)DownloadImage{
    if (self.downloadBlock) {
        self.downloadBlock(self.currentImageIndex);
    }
}
