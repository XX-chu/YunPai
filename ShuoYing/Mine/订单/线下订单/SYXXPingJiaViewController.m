...
@interface SYXXPingJiaViewController ()<UITableViewDelegate, UITableViewDataSource, YBAttributeTapActionDelegate>
{
    NSInteger _count;
}
...
@implementation SYXXPingJiaViewController
...
- (void)getData{
    ...
    NSDictionary *param = @{@"token":UserToken, @"state":@24, @"page":@1};
    __weak typeof(self)weakself = self;
    [[SYHttpRequest sharedInstance] getDataWithUrl:url Parameter:param ResponseObject:^(NSDictionary *responseResult) {
        [SVProgressHUD dismiss];
...
- (void)getMoreData{
    _count ++;
    NSString *url = [NSString stringWithFormat:@"%@%@",BaseUrl,@"/order/all.html"];
    NSDictionary *param = @{@"token":UserToken, @"state":@24, @"page":[NSNumber numberWithInteger:_count]};
    [[SYHttpRequest sharedInstance] getDataWithUrl:url Parameter:param ResponseObject:^(NSDictionary *responseResult) {
