...
@interface SYXSShouHuoViewController ()<UITableViewDelegate, UITableViewDataSource>
{
    NSInteger _count;
}
...
@implementation SYXSShouHuoViewController
...
- (void)getData{
    ...
    NSDictionary *param = @{@"token":UserToken, @"state":@13, @"page":@1};
    __weak typeof(self)weakself = self;
    [[SYHttpRequest sharedInstance] getDataWithUrl:url Parameter:param ResponseObject:^(NSDictionary *responseResult) {
        [SVProgressHUD dismiss];
...
- (void)getMoreData{
    _count ++;
    NSString *url = [NSString stringWithFormat:@"%@%@",BaseUrl,@"/order/all.html"];
    NSDictionary *param = @{@"token":UserToken, @"state":@13, @"page":[NSNumber numberWithInteger:_count]};
    [[SYHttpRequest sharedInstance] getDataWithUrl:url Parameter:param ResponseObject:^(NSDictionary *responseResult) {
