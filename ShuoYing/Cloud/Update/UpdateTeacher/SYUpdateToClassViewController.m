//
//  SYUpdateToClassViewController.m
//  ShuoYing
//
...
@interface SYUpdateToClassViewController ()<XLPhotoBrowserDelegate,XLPhotoBrowserDatasource>
{
...
@implementation SYUpdateToClassViewController
...
- (void)updatePhotos:(UIButton *)sender{
    ...
    if (self.selectedImagesArr.count == 0) {
        [self showHint:@"请选择您要上传的照片"];
        return;
    }
    if (_moneyTF.text.length == 0) {
        [self showHint:@"请填写您要上传照片的价格"];
        return;
    }
    ...
    NSDictionary *param = @{@"token":UserToken, @"id":self.classID, @"money":_moneyTF.text};
    [[XBUpdatePhotoManager sharedInstance] updatePhotosWithFile:[NSString stringWithFormat:@"%@%@",BaseUrl, @"/teacher/upclassX.html"] param:param fileData:data name:@"img" fileName:[NSString stringWithFormat:@"img%lu.%@",(long)currentNum, [mimaType substringWithRange:NSMakeRange(6, mimaType.length - 6)]] mimeType:mimaType currentNumber:currentNum allNumber:self.selectedPhotoModelsArr.count result:^(NSDictionary *dic) {
...
- (void)initUI{
    ...
    UITextField *moneyTF = [[UITextField alloc] initWithFrame:CGRectMake(15, 5, kScreenWidth - 30, 30)];
    moneyTF.placeholder = @"请输入单张图片金额（元）";
    moneyTF.borderStyle = UITextBorderStyleNone;
    _moneyTF = moneyTF;
    [self.backView addSubview:moneyTF];
    
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(moneyTF.frame) + 5, kScreenWidth, 1)];
    lineView.backgroundColor = RGB(242, 242, 242);
    [self.backView addSubview:lineView];
    
    UIButton *selectBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    selectBtn.frame = CGRectMake(15, CGRectGetMaxY(lineView.frame) + 10, 75, 75);
