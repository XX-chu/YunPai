...
@interface SYGroupPhotoViewController ()<XLPhotoBrowserDelegate,XLPhotoBrowserDatasource>
...
@implementation SYGroupPhotoViewController
...
- (void)updatePhotos:(UIButton *)sender{
    ...
    NSDictionary *param = @{@"token":UserToken, @"id":self.groupID, @"money":_moneyTF.text};
    [[XBUpdatePhotoManager sharedInstance] updatePhotosWithFile:[NSString stringWithFormat:@"%@%@",BaseUrl, @"/group/upgroup.html"] param:param fileData:data name:@"img" fileName:[NSString stringWithFormat:@"img%lu.%@",(long)currentNum, [mimaType substringWithRange:NSMakeRange(6, mimaType.length - 6)]] mimeType:mimaType currentNumber:currentNum allNumber:self.selectedPhotoModelsArr.count result:^(NSDictionary *dic) {
